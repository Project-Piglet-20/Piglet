...
class Table extends Component {
    ...
    clickHandler = (event) => {
        ...
        var newIssues = this.state.issues.filter(issue => issue.DocID !== event)
        ...
        const issueList = this.state.issues.map(issue => {
            return (
                <tr key= { ++id }>
                    <td> {'\xa0\xa0'} { id } </td>
                    <td> {'\xa0\xa0'} { issue.Category } </td>
                    <td> {'\xa0\xa0'} { issue.Type } </td>
                    <td> {'\xa0\xa0'} { issue.Locality } </td>
                    <td> {'\xa0\xa0'} { issue.DOR } </td>
                    <td className= "center"> <button className= { issue.Status === "Resolved" ? "btn waves-effect waves-teal" : "btn waves-effect waves-red red"} value= { issue.Status } 
                        onClick= { () => this.clickHandler(issue.DocID) }
                        > { issue.Status } </button> </td>
                    <td> {'\xa0\xa0'} { issue.DOC } </td>
                </tr>
            )
        })
                        ...
                        const issueList = issues.map(issue => {
                            return (
                                <tr key= { ++id }>
                                    ...
                                    <td> {'\xa0\xa0'} { issue.Locality } </td>
                                    <td> {'\xa0\xa0'} { issue.DOR } </td>
                                    <td> <button className= 'btn waves-effect waves-red red'
                                        onClick= { () => this.clickHandler(issue.DocID) }
                                        > { issue.Status } </button> </td>
                                    <td> {'\xa0\xa0'} { issue.DOC } </td>
                                </tr>
    ...
    render() {
        return (
            <div>
                <div>
                    <h3> <i className="material-icons medium left" style= {{color: 'teal'}}>account_circle</i> Corporator <button className="btn waves-effect waves-teal right" onClick= { this.logoutHandler }> Logout </button></h3>
                </div>
                <br/>
                <Heatmap center = { this.props.center } />
                <br/>
